...
blockchain project/
├── contracts/
│   └── PharmSupplyChain.sol        ← Smart contract (Solidity)
├── scripts/
│   └── deploy.js                   ← Deploys contract to blockchain
├── test/
│   └── PharmSupplyChain.test.js    ← 29 smart contract tests
├── hardhat.config.js               ← Blockchain tooling config
│
├── backend/
│   ├── server.js                   ← Express app entry point
│   ├── config/
│   │   ├── blockchain.js           ← Ethers.js connection to contract
│   │   └── db.js                   ← MongoDB connection
│   ├── middleware/
│   │   ├── auth.js                 ← JWT token verification
│   │   └── roleCheck.js            ← Role-based access control
│   ├── models/
│   │   ├── User.js                 ← User accounts (with wallet address)
│   │   ├── BatchMetadata.js        ← Off-chain medicine details
│   │   ├── UnitRecord.js           ← Individual unit/packet tracking
│   │   ├── PackagingLevel.js       ← Box/carton hierarchy
│   │   └── ScanLog.js              ← QR scan audit trail
│   ├── services/
│   │   ├── blockchainService.js    ← Smart contract interaction wrapper
│   │   ├── unitService.js          ← Merkle tree + unit verification logic
│   │   ├── qrService.js            ← QR code generation
│   │   └── anomalyService.js       ← Suspicious activity detection
│   └── routes/
│       ├── auth.js                 ← Login/register endpoints
│       ├── batch.js                ← Batch CRUD + transfer
│       ├── unit.js                 ← Unit verification + packaging scan
│       ├── verify.js               ← Consumer scan endpoint
...
│       │   └── blockchain.js       ← Direct blockchain reads from frontend
│       ├── pages/
│       │   ├── Login.jsx           ← Auth page
│       │   ├── Register.jsx        ← Account creation
│       │   ├── Dashboard.jsx       ← Role-based dashboard router
│       │   ├── ManufacturerDashboard.jsx  ← Create batches with packaging config
│       │   ├── DistributorDashboard.jsx   ← Manage transfers
│       │   ├── PharmacyDashboard.jsx      ← Receive shipments, see unit stats
│       │   ├── ConsumerDashboard.jsx      ← Scan & verify medicine
│       │   ├── AdminDashboard.jsx         ← System management
│       │   └── VerifyMedicine.jsx         ← Batch + unit verification UI
│       └── components/
│           ├── QRScanner.jsx       ← Camera + text input scanner
│           ├── BatchCard.jsx       ← Batch display card
│           ├── SupplyChainTimeline.jsx  ← Visual transfer history
│           └── TransferForm.jsx    ← Batch transfer modal
```
...
> A regular database is controlled by one company — they could change records. Blockchain is **decentralized and immutable**. Once a batch is registered, nobody (not even the admin) can alter its creation timestamp, manufacturer, or Merkle root. This creates a trustless system where no single party needs to be trusted.

**Q: Why not store everything on the blockchain?**
> Cost and speed. On Ethereum mainnet, storing 1 KB costs ~$5-50 in gas fees. A single batch's metadata (name, description, side effects) could be 1-2 KB. For 10,000 units, that's millions of dollars. We store only the **critical proof data** (32-byte hashes) on-chain and everything else in MongoDB.

**Q: What is gas? How do transactions cost money?**
> Gas is the fee you pay to miners/validators for executing your transaction. Every operation in a smart contract (storing data, computing, etc.) costs a certain amount of gas. We use Hardhat's local network where gas is free for development.

**Q: What happens if the Hardhat node goes down?**
> The local blockchain state is lost. In production, you'd deploy to a **persistent network** (Ethereum mainnet, Polygon, or a private enterprise chain). For our demo, we redeploy and re-register stakeholders when restarting.

**Q: Can the smart contract be updated/changed?**
> No. Once deployed, the code is **immutable**. If we need changes, we deploy a new contract and migrate data. This is a feature, not a bug — it guarantees the rules can't be tampered with. (Note: upgradeable proxy patterns exist but add complexity).

---

### Merkle Tree Questions:

**Q: Why not just store all serial numbers on the blockchain?**
> For 10,000 units, that's 10,000 × 32 bytes = 320 KB on-chain. At current Ethereum prices, that could cost thousands of dollars. With a Merkle tree, we store just ONE 32-byte hash (the root) and can still prove any individual unit belongs to the batch.

**Q: How is the Merkle proof generated?**
> When we build the tree, we save each unit's **proof path** (the sibling hashes needed to recompute up to the root). For 10,000 units, each proof is ~13 hashes (log₂(10000)). These proofs are stored in MongoDB alongside each unit.

**Q: What if someone creates a fake serial number?**
> The fake serial number's hash won't exist as a leaf in the Merkle tree. When they try to verify it, the proof computation will produce a different root than what's stored on the blockchain → **verification fails** → flagged as counterfeit.
...
> MongoDB's flexible schema lets us store different types of data (users, batches, units, scan logs) without rigid table structures. Its JSON-like documents map naturally to our JavaScript objects. Also, the `insertMany()` operation is very fast for bulk-inserting thousands of unit records.

**Q: Why React + Vite instead of plain HTML?**
> React's component model lets us build reusable UI pieces (BatchCard, QRScanner, Timeline). Vite provides instant hot-reload during development. The role-based dashboard system would be very complex with plain HTML.

**Q: How does the QR scanner work?**
> We use the `html5-qrcode` library which accesses the device camera via the WebRTC API. When a QR code is detected, it extracts the URL, parses it to determine if it's a batch, unit, or packaging code, and routes accordingly.
...
> Passwords are hashed using **bcrypt** with 12 salt rounds. We never store plain text passwords. Even if the database is breached, attackers can't reverse the hash to get passwords.

**Q: What prevents someone from forging a JWT token?**
> JWT tokens are signed with a server-side secret key (`JWT_SECRET`). Without this key, you can't create a valid token. The server verifies the signature on every request.

**Q: How do you prevent QR code cloning?**
> Multiple layers: (1) **Scan count tracking** — if a unit is scanned >5 times, it's flagged. (2) **Dispensed status** — once a consumer scans it, it's marked "dispensed"; a second scan shows "Already Sold". (3) **Anomaly detection** — multiple locations, IPs, and rapid scans are detected. (4) **Merkle proof** — a cloned QR with a fake serial number fails the cryptographic proof.

**Q: Is storing private keys in the database safe?**
> **No, absolutely not.** This is a development shortcut. In production, users would sign transactions using **MetaMask** or a hardware wallet. The private key would never leave the user's device.
...
**Q: What is keccak256?**
> It's the hashing algorithm used by Ethereum. It takes any input and produces a fixed 32-byte (256-bit) output. It's one-way (can't reverse it) and collision-resistant (practically impossible to find two inputs with the same hash).

**Q: What is Ethers.js?**
> It's a JavaScript library that lets our Node.js backend communicate with the Ethereum blockchain. It can: create wallets, sign transactions, call smart contract functions, and read blockchain data.

**Q: What are the `modifiers` in the smart contract?**
> Modifiers are reusable conditions that run before a function executes. For example, `onlyAdmin` checks if the caller has the Admin role. If not, the transaction **reverts** (fails and refunds gas). This is how we enforce access control.

**Q: How does the cascade update work technically?**
> When a carton is scanned, we: (1) Find all boxes with `parentId = cartonId` using MongoDB query. (2) Collect all box IDs. (3) Use `updateMany({ parentBoxId: { $in: boxIds } })` to update all units in one database operation. MongoDB handles this efficiently.

---

### Project-Specific Questions:

**Q: What are the 5 roles and what can each do?**
> | Role | Permissions |
> |---|---|
> | **Admin** | Register stakeholders, deactivate batches, view all data |
> | **Manufacturer** | Create batches, transfer to distributors, view own batches |
> | **Distributor** | Receive from manufacturers, transfer to pharmacies, scan packaging |
> | **Pharmacy** | Receive from distributors, scan incoming shipments, dispense to consumers |
> | **Consumer** | Scan and verify individual medicine packets (no login required) |

**Q: What happens when a consumer scans a medicine?**
> 5-step verification: (1) Does the serial number exist in our database? (2) Has it already been sold? (3) Does the cryptographic Merkle proof match the blockchain? (4) Are there any scan anomalies? (5) Mark as dispensed and return the full result with medicine details and supply chain history.

**Q: How many tests does the project have?**
> **29 smart contract tests**, covering: deployment, stakeholder registration, batch creation (with Merkle root), batch transfer (full supply chain flow), verification, Merkle root storage, and batch deactivation. All 29 pass.

**Q: What would you change for production deployment?**
> 1. Deploy to a real blockchain (Polygon for low fees, or a private Hyperledger chain)
> 2. Use MetaMask for transaction signing (no private keys on server)
> 3. Add HTTPS and proper CORS configuration
...
|---|---|
| **Smart Contract** | A program on the blockchain that runs automatically, can't be changed |
| **Gas** | The fee for blockchain transactions (like postage for mail) |
| **Wallet Address** | Like a bank account number for blockchain (0x...) |
| **Private Key** | Like a password for your wallet — signs transactions |
| **Transaction** | A write operation on the blockchain (costs gas) |
| **Block** | A group of transactions packaged together |
| **Hash** | A fixed-length fingerprint of any data (one-way, unique) |
| **Merkle Tree** | A tree of hashes that lets you prove membership efficiently |
| **Merkle Root** | The single top hash that represents all data in the tree |
| **Merkle Proof** | The minimum set of hashes needed to verify one item |
| **ABI** | Application Binary Interface — the "API definition" of a smart contract |
| **Ethers.js** | JavaScript library to talk to Ethereum |
| **JWT** | JSON Web Token — a signed token for API authentication |
| **bcrypt** | A slow hashing algorithm designed for password security |
...
| **Hardhat** | Development tool for compiling, testing, and running Ethereum locally |

---

## 14. ONE-LINE PROJECT SUMMARY

> **"A blockchain-based pharmaceutical supply chain system that uses Ethereum smart contracts for immutable batch tracking, Merkle trees for individual packet verification, and hierarchical QR codes with cascade scanning to enable consumers to verify medicine authenticity while minimizing manual work for supply chain participants."**
